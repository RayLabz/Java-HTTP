...
//                .send();
        ...
        BasicHTTPRequest.create("http://localhost:8080/api/helloTest", RequestMethod.POST)
                .addParam("a", 1)
                .addParam("b", 2)
                .onSuccess(response -> {
                    if (response.isSuccess()) {
                        System.out.println(response.getContent());
                        System.out.println("Latency: " + response.getLatency() + "ms");
                    }
                    else {
        ...
        BasicHTTPRequest.create("http://localhost:8080/api/helloGet", RequestMethod.GET)
                .addParam("a", 1)
                .addParam("b", 2)
                .onSuccess(response -> {
                    if (response.isSuccess()) {
        ...
        BinaryHTTPRequest.create("http://localhost:8080/api/binaryTest")
                .putString("Nicos")
                .putInt(1993)
                .onSuccess(response -> {
                    if (response.isSuccess()) {
                        final String message = response.readString();
                        final int responseNumber = response.readInt();
